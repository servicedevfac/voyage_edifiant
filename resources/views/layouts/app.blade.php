...
<html lang="fr">
    <head>
        ...
        <title>@hasSection('meta-title')@yield('meta-title')@else@yield('title', 'Voyage Édifiant | Voyages, excursions & location de voiture en Côte d’Ivoire')@endif - Voyage Edifiant</title>
        <meta name="description" content="@yield('meta-description', 'Découvrez la Côte d\'Ivoire avec Voyage Édifiant : excursions privées, voyages sur mesure, location de voiture avec chauffeur à Abidjan et dans tout le pays.')">
        <meta name="keywords" content="@yield('meta-keywords', 'voyage Côte d\'Ivoire, excursions Abidjan, location voiture Abidjan, chauffeur privé, tourisme Côte d\'Ivoire, Voyage Édifiant')">
        <meta property="og:title" content="@hasSection('meta-title')@yield('meta-title')@else@yield('title', 'Voyage Édifiant')@endif">
        <meta property="og:description" content="@yield('meta-description', 'Découvrez la Côte d\'Ivoire avec Voyage Édifiant')">
        <meta property="og:image" content="@yield('meta-image', asset('assets/front/images/bg-travel.png'))">
    ...
    <body>
            ...
            <nav class="navbar">
                <div class="logo">
                    <a href="{{ route('accueil') }}">
                        <img src="{{ url('assets/front/images/logo.png') }}" alt="Logo">
                    </a>
                </div>
                <ul class="nav-links">
        ...
        @if(!View::hasSection('hide-global-alert'))
            @if(session('success') || session('status') || session('error') || $errors->any())
                <div class="container mt-4 mb-2" style="position: relative; z-index: 9999;">
                    @if(session('success'))
                ...
                <script>
                    // Faire défiler automatiquement vers le message d'alerte pour que l'utilisateur le voie immédiatement
        ...
        <script>
            ...
            if (document.querySelector("#dateDebut") && document.querySelector("#dateFin")) {
